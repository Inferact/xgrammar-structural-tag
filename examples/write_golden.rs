...
fn strict_false_tool(name: &str) -> ToolParam {
    ToolParam::Function(
        FunctionToolParam::new(FunctionDefinition::new(name).with_parameters(json!({
            "type": "object",
            "properties": { "q": { "type": "string" } },
            "required": ["q"]
        })))
...
trait StrictFalse {
    fn tap_strict_false(self) -> Self;
}

impl StrictFalse for FunctionToolParam {
    fn tap_strict_false(mut self) -> Self {
        self.function.strict = Some(false);
        self
    }
}
